...
namespace Be.Stateless.BizTalk.Dsl.Pipeline.CodeDom
{
	public class PipelineExtensionsFixture
	{
		...
		public void FFReceiveCodeCompileUnitCompiles()
		{
			...
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new FFReceive().ConvertToCodeCompileUnit());
		...
		public void FFTransmitCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new FFTransmit().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
		...
		public void PassThruReceiveCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new PassThruReceive().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
		...
		public void PassThruTransmitCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new PassThruTransmit().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
		...
		public void XmlMicroPipelineCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new XmlMicroPipeline().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
			...
			Regex.Replace(builder.ToString(), @"(//\s+)Runtime Version:\d\.\d\.\d+\.\d+", @"$1Runtime Version:4.0.30319.42000", RegexOptions.Multiline)
				.Should().Be(
					ResourceManager.Load(
						...
						"Be.Stateless.BizTalk.Dsl.Pipeline.Resources.XmlMicroPipeline.btp.cs",
		...
		public void XmlReceiveCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new XmlReceive().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
		...
		public void XmlRegularPipelineCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new XmlRegularPipeline().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
		...
		public void XmlTransmitCodeCompileUnitCompiles()
		{
			using (var provider = new CSharpCodeProvider())
			{
				var results = provider.CompileAssemblyFromDom(CompilerParameters, new XmlTransmit().ConvertToCodeCompileUnit());
				results.Errors.HasErrors.Should().BeFalse();
				results.Errors.Should().BeEmpty();
			}
		}
